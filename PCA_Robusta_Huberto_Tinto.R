...
pc.ROBPCA <- PcaHubert(data_standardizada, kmax = ncol(data_standardizada), k = 4, crit.pca.distances = 0.99999)
summary(pc.ROBPCA)
...
title("ROBPCA, k=4",cex=0.8)


round(pc.ROBPCA$loadings,5)
...
cat("Accuracy Naive Bayes PCA ROBUSTA Red wine =" , cmx$overall[1])
...
cmx <- confusionMatrix(naive_pred, test_cl$Quality)
cat("Accuracy Naive Bayes PCA ROBUSTA Red Wine Considerando Certo quando 2 Classes têm P >0.4 = =" , cmx$overall[1])
...
cat("Accuracy Naive Bayes PCA ROBUSTA Red wine Output 0,1,2 =" , cmx$overall[1])
...
svmfit = svm(train_cl$Quality ~ ., data = train_cl, kernel = "radial", cost = 10^(-2), gamma = 10, scale = FALSE, probability = TRUE, cross = 5)
...
cat("Accuracy SVM PCA ROBUSTA Red wine =" , cmx$overall[1])
...
cat("Accuracy SVM PCA ROBUSTA Red wine Considerando Certo quando 2 Classes têm P >0.4 =" , cmx$overall[1])
...
svmfit = svm(train_cl$Good_Bad ~ ., data = train_cl, kernel = "radial", cost = 10^(-2), gamma = 10, scale = FALSE, probability = TRUE,cross = 5)
y_pred_SVM <- predict(svmfit, newdata = test_cl,decision.values = TRUE, probability = TRUE) 
...
cat("Accuracy SVM PCA ROBUSTA Red wine Output 0,1,2 =" , cmx$overall[1])
...
cat("Accuracy LDA PCA ROBUSTA Red wine =" , cmx$overall[1])
...
cat("Accuracy LDA PCA ROBUSTA Red wine Output Classes 0,1,2 =" , cmx$overall[1])
...
cmx <- confusionMatrix(rf_pred, test_cl$Quality)
cat("Accuracy RF Red Wine PCA Robusta Considerando Certo quando 2 Classes têm P >0.4 = =" , cmx$overall[1])
...
cat("Accuracy RF PCA Robusta Red wine Output Classes 0,1,2 =" , cmx$overall[1])
